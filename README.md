# <img src="logo.png" alt="Logo QuantImport" width="70"> [QuantImport](https://quantimportbrazil.github.io/Sobre/)

---

# Fator mais Importante - Exemplo

> **Emissão:** 10-2025  
> O aprendizado de máquina (ML) não só faz previsões, mas também permite ranquear a importância dos fatores que as afetam.

---

**[Home](https://quantimportbrazil.github.io/Sobre/)**  

Para demonstrar essa capacidade, dentre as previsões demonstrativas, escolhemos aquela que apresentou a maior variação em relação ao ano anterior:  
* Produto SH4 3102 — Município de Rondonópolis-MT — Mês 09/2025 para o 09/2026

Essa grande variação entre uma previsão e outra pode ser observada no gráfico a seguir:

![Gráfico de Previsão](3102.png)

# Note que, mesmo tendo a importação real tendo sido maior do que a importação prevista para o mês 09 de 2025, o ML previu um valor 50 mil toneladas ainda menor para 09 de 2026!

# A razão apontada pelo ML para essa redução foi a queda nas exportação apresentada no quadro abaixo::

![Fator mais Importante](fator_mais_importante.png)  


---

## Contato
**André Coutinho Bueno**  
Cientista de Dados e Fundador  
[andre.bueno@quantimport.com.br](mailto:andre.bueno@quantimport.com.br)
